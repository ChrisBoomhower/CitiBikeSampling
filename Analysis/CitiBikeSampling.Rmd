...
mydesign <- svydesign(id = ~1, strata = ~DayOfWeek + TimeOfDay, data = pop.data.PropSampled)
...
```{r}
  # Create SurveyDesign
mydesign <- svydesign(id = ~1, strata = ~DayOfWeek + TimeOfDay, data = NeySamp) # Dropped since no correction needed: , fpc = ~Freq)
...
$deff_{complex}=\frac{V(\bar{y}_{complex})}{V( \bar{y} {srs)})}=\frac{8.2456}{8.2137}=0.0722$
...
$n_{0,complex} = n_{0,srs} \times deff_{complex}= 33474 \times 1.004 = 33601.98 \approx 33602$
